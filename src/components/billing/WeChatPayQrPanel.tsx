...
export function WeChatPayQrPanel({ amountCny, orderId, onSuccess, onFailure, onSwitchMethod, forceFail }: Props) {
  ...
  return (
    <div className="mx-auto max-w-[400px] rounded-xl border border-ink-200 bg-surface-0 p-6 text-center shadow-lg">
      <div className="flex items-center justify-center gap-3">
        <span className="size-3 rounded-full" style={{ backgroundColor: "#07C160" }} aria-hidden="true" />
        <h2 className="text-lg font-bold text-ink-900">Pay with WeChat Pay</h2>
      </div>
      <p className="mt-2 text-sm text-ink-500">{copy.payment.wechat.instructions}</p>
      <div className="relative mx-auto mt-6 w-fit rounded-lg bg-ink-50 p-4">
        ...
        <div className="rounded-md bg-surface-0 p-3">
          <QRCodeSVG value={`weixin://nexhibit/pay/${orderId}`} size={240} marginSize={2} />
        </div>
      </div>
      ...
      <p className="mt-2 text-sm text-ink-500">{waiting ? copy.payment.wechat.qrPrompt : copy.payment.states.processing}</p>
      <Button
        type="button"
