...
import { useStudentMessagesStore } from "@/stores/student-messages-store";
...
const quickReplies = [
  {
    label: "Thanks",
    text: "Thanks for reaching out. I appreciate you reviewing my profile and would be happy to share more context.",
  },
  {
    label: "Schedule call",
    text: "I would be happy to schedule a call. I am available Friday afternoon or Monday morning and can adjust if another time works better.",
  },
  {
    label: "Send resume",
    text: "I can send my resume and a short project evidence note here. Please let me know if you prefer PDF or portfolio links.",
  },
  {
    label: "Polite decline",
    text: "Thank you for considering me. I do not think this role is the right fit right now, but I appreciate the conversation.",
  },
];
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-1">
                      <div className="flex min-w-0 items-center gap-1.5">
                        ...
                        {employer.verified ? <VerifiedBadge className="hidden sm:inline-flex" /> : null}
                      </div>
        ...
        {activeConv && activeEmployer ? (
          <>
            ...
            <div className="flex items-center justify-between border-b border-ink-200 px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="flex items-center gap-1.5 text-sm font-semibold text-ink-900">
                    {activeEmployer.name}
                    {activeEmployer.verified ? <VerifiedBadge /> : null}
                  </p>
                  <Link
              ...
              <div className="flex items-center gap-3">
                <label className="hidden items-center gap-2 text-xs font-semibold text-ink-600 md:flex">
                  Show Thai/English translation
                  <Switch checked={showTranslation} onCheckedChange={setShowTranslation} aria-label="Show Thai/English translation" />
                </label>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-64">
                    <DropdownMenuItem onSelect={(event) => event.preventDefault()} className="flex items-center justify-between gap-3">
                      <span>Read receipts</span>
                      <Switch checked={readReceipts} onCheckedChange={setReadReceipts} aria-label="Toggle read receipts" />
                    </DropdownMenuItem>
                    <DropdownMenuItem onSelect={(event) => event.preventDefault()} className="flex items-center justify-between gap-3 md:hidden">
            ...
            {!calendarBannerDismissed ? (
              <div className="border-b border-gold-200 bg-gold-50 px-4 py-3">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-start gap-2 text-sm text-ink-800">
                    <CalendarCheck className="mt-0.5 size-4 shrink-0 text-gold-700" aria-hidden="true" />
                    <span>Connect Google Calendar to auto-schedule meetings.</span>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="primary" size="sm" onClick={() => toast.info("Google Calendar connection is mocked in this prototype.")}>Connect</Button>
                    <Button variant="ghost" size="sm" onClick={dismissCalendarBanner}>Dismiss</Button>
                  </div>
                </div>
              </div>
            ) : null}
                ...
                return (
                  <div key={msg.id} className={cn("flex", isStudent ? "justify-end" : "justify-start")}>
                    ...
                    >
                      {msg.text}
                      {showTranslation ? (
                        <span className={cn("mt-1 block text-xs italic", isStudent ? "text-ink-300" : "text-ink-500")}>
                          {mockTranslation(msg.text)}
                        </span>
                      ) : null}
                      {isStudent && readReceipts ? <span className="mt-1 block text-[10px] text-ink-300">Read</span> : null}
                    </div>
                    {isStudent && (
            ...
            <div className="border-t border-ink-200 px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => toast.info("Attachment action is simulated in this prototype.")}
                >
                ...
                <Button
                  ...
                  onClick={() => {
                    setDraft("");
                    toast.success("Demo reply sent for this prototype session.");
                  }}
